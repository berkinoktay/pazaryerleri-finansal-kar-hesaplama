...
- **`GET /v1/organizations/{orgId}/stores/{storeId}/live-performance/kpis`** — today vs. yesterday KPI summary (revenue, net profit, order count, margin). Store-scoped (`requireStoreAccess`); aggregates the orders table over the business-day window. Money values are Decimal strings.
- **`GET /v1/organizations/{orgId}/stores/{storeId}/live-performance/chart`** — hourly cumulative net-profit curve for today and yesterday (24 business-hour buckets, for a dual-line intraday chart).
- **`GET /v1/organizations/{orgId}/stores/{storeId}/live-performance/missing-cost`** — variants in today's PENDING buffer that still lack a cost profile, grouped by barcode with order count + blocked revenue. Already-costed siblings of a still-missing line are excluded.
- **`GET /v1/organizations/{orgId}/stores/{storeId}/live-performance/top-products`** — today's top 3 best-selling variants by order count, with revenue and best-effort profit.
