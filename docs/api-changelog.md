...
- **Discount commission bands now resolve by the covering tariff week on the current path, and the detail flags an expired tariff week** (`GET /v1/organizations/{orgId}/stores/{storeId}/discount-lists/{listId}`). The covering-week lookup runs for EVERY anchor — the anlık `now` path included, not just future starts — so with two adjacent weeks uploaded out of order the tariff whose week covers today wins over whichever tariff has the newest `createdAt`; only when no week covers the anchor does it fall back to the latest-uploaded tariff's last-available period. `DiscountListDetail` gains a required boolean `commissionPeriodExpired` — true only when that fallback landed on a period that already ended (the current week's tariff is not uploaded yet), false when the resolved period is current/future or no tariff resolved. The estimate endpoint is unchanged.
