...
import { exportAdvantageTariff } from '../../services/advantage-tariff-export.service';
...
const exportAdvantageTariffRoute = createRoute({
  method: 'post',
  path: '/organizations/{orgId}/stores/{storeId}/advantage-tariffs/{tariffId}/export',
  tags: ['AdvantageTariffs'],
  summary: 'Export the Advantage tariff as a re-uploadable Trendyol Excel',
  description:
    "Returns Trendyol's ORIGINAL uploaded file with the seller's chosen tier price patched into the " +
    '"YENİ TSF" column for every product that has a selected tier. Every other cell is byte-for-byte ' +
    'unchanged, so the file can be re-uploaded to Trendyol verbatim. Marks the tariff exported. ' +
    'Returns 409 if the tariff has no stored source file (or it is unreadable / lacks the writable ' +
    'column).',
  security: [{ bearerAuth: [] }],
...
app.openapi(exportAdvantageTariffRoute, async (c) => {
  ...
  const { file, filename } = await exportAdvantageTariff(orgId, storeId, tariffId);
