// Focused integration test for the commission-tariff import's one-tariff-per-week
// guard. A commission upload's week window is [min period start … max period end].
// A new upload whose window OVERLAPS an existing tariff's window for the store is
// rejected (422 VALIDATION_ERROR, field `file`, code `DUPLICATE_TARIFF_WEEK`).
// Back-to-back weeks only TOUCH (the previous week ends 07.59, the next starts
// 08.00 — a 1-minute gap), so they do NOT overlap and are allowed.
//
// The real Trendyol fixture's single period is "26 Haziran 08.00-30 Haziran 07.59",
// which parses to a concrete week window — so uploading it twice into one store is
// a guaranteed self-overlap, and a neighbour ending exactly where it begins is the
// back-to-back boundary case.
...
function importRequest(ctx: Ctx): Request {
  ...
  form.append('file', new Blob([new Uint8Array(FIXTURE)]), 'trendyol-commission-tariff.xlsx');
  return new Request(
...
describe('commission-tariffs import — one-tariff-per-week overlap guard', () => {
  beforeEach(async () => {
  ...
  it('allows a back-to-back (1-minute-gap) week', async () => {
    ...
    const window = await prisma.commissionTariff.findUnique({
      where: { id: tariffId },
    ...
    // A fresh store holds ONLY the PREVIOUS week, ending one minute before this
    // week begins (…07.59 vs …08.00). Adjacent ranges touch but do not overlap.
    const ctx = await setupStore();
    await prisma.commissionTariff.create({
      data: {
