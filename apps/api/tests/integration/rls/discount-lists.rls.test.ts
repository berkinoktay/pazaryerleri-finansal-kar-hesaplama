...
describe('RLS — discount_lists (+ items)', () => {
  beforeAll(async () => {
  ...
  // valid OWNER'd tenant whose private Discounts data userA must not see.
  ...
  async function seedDiscountChain(organizationId: string, storeId: string) {
  ...
  it('org A user CANNOT read org B Discounts chain', async () => {
    const { client, orgB, storeB } = await twoTenantsSetup();
    await seedDiscountChain(orgB.id, storeB.id);

    const lists = await client.from('discount_lists').select('id');
    const items = await client.from('discount_list_items').select('id');
  ...
  it('org A user CAN read own org A Discounts chain', async () => {
    const { client, orgA, storeA } = await twoTenantsSetup();
    const { list, item } = await seedDiscountChain(orgA.id, storeA.id);

    const lists = await client.from('discount_lists').select('id,name');
    const items = await client.from('discount_list_items').select('id,barcode');
