...
import { useExportPlusTariff } from '@/features/campaigns/hooks/use-export-plus-tariff';
...
const ENDPOINT = `http://localhost:3001/v1/organizations/${ORG_ID}/stores/${STORE_ID}/plus-commission-tariffs/${TARIFF_ID}/export`;
...
describe('useExportPlusTariff', () => {
  it('resolves with the patched file bytes + the server-chosen filename on success', async () => {
    ...
    const { result } = renderHook(() => useExportPlusTariff(ORG_ID, STORE_ID), {
