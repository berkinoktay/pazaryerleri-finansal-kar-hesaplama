...
import { useDeletePlusTariff } from '@/features/campaigns/hooks/use-delete-plus-tariff';
...
describe('useDeletePlusTariff', () => {
  it('resolves on 204 and invalidates the store Plus tariff list', async () => {
    server.use(http.delete(ENDPOINT, () => new HttpResponse(null, { status: 204 })));

    const client = createTestQueryClient();
    const invalidateSpy = vi.spyOn(client, 'invalidateQueries');

    const { result } = renderHook(() => useDeletePlusTariff(ORG_ID, STORE_ID), {
