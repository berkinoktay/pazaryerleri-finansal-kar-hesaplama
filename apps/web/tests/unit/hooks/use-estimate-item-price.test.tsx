...
import { useEstimateItemPrice } from '@/features/campaigns/hooks/use-estimate-item-price';
...
const ENDPOINT = `http://localhost:3001/v1/organizations/${ORG_ID}/stores/${STORE_ID}/commission-tariffs/${TARIFF_ID}/items/${ITEM_ID}/estimate`;
...
describe('useEstimateItemPrice', () => {
  it('resolves with the full breakdown when calculable=true', async () => {
    ...
    const { result } = renderHook(() => useEstimateItemPrice(ORG_ID, STORE_ID, TARIFF_ID), {
      wrapper,
    });
    result.current.mutate({ itemId: ITEM_ID, body: { price: '450.00', bandKey: 'band1' } });
  ...
  it('sends the typed price + bandKey in the request body (band-click mode)', async () => {
    ...
    result.current.mutate({ itemId: ITEM_ID, body: { price: '400.00', bandKey: 'band2' } });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(capturedBody).toEqual({ price: '400.00', bandKey: 'band2' });
  });
});
