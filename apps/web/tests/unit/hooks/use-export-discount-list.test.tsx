...
import { useExportDiscountList } from '@/features/campaigns/hooks/use-export-discount-list';
...
const ENDPOINT = `http://localhost:3001/v1/organizations/${ORG_ID}/stores/${STORE_ID}/discount-lists/${LIST_ID}/export`;
...
describe('useExportDiscountList', () => {
  it('resolves with the patched file bytes + the server-chosen filename (RFC 5987) on success', async () => {
        ...
        () =>
          new HttpResponse(XLSX_BYTES, {
            ...
            headers: {
              ...
              // Turkish name percent-encoded in the RFC 5987 `filename*` form — the parser must
              // decode it back to the accented original (%C4%B0 = "İ").
              'content-disposition': "attachment; filename*=UTF-8''%C4%B0ndirimler.xlsx",
            },
          }),
      ),
    );

    const { result } = renderHook(() => useExportDiscountList(ORG_ID, STORE_ID), {
    ...
    expect(result.current.data?.filename).toBe('İndirimler.xlsx');
  });
  ...
  it('throws an ApiError when the endpoint returns a JSON problem (409 no source file)', async () => {
    server.use(
      http.post(ENDPOINT, () =>
        HttpResponse.json(
          {
