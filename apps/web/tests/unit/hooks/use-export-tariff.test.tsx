...
describe('useExportTariff', () => {
  it('resolves with the patched file bytes + the server filename on success', async () => {
    server.use(
      http.post(
        ENDPOINT,
        () =>
          new HttpResponse(XLSX_BYTES, {
            ...
            headers: {
              ...
              'content-disposition': "attachment; filename*=UTF-8''tarife.xlsx",
            },
          }),
      ),
    );
    ...
    expect(result.current.data?.filename).toBe('tarife.xlsx');
  });
