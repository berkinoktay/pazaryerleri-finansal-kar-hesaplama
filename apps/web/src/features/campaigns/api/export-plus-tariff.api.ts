 ...
 *
 * Returns the ORIGINAL Trendyol Plus `.xlsx` with the seller's opt-in choices
 * patched in, ready to re-upload verbatim. Resolves to the file `Blob`.
 *
 * Binary-download convention: the endpoint returns an xlsx on success but a JSON
 * ProblemDetails on error, so a blanket `parseAs` would misparse one of them. We
 * use `parseAs: 'stream'` (openapi-fetch does NOT consume or JSON-parse the body),
 * then branch on `response.ok`: on success read the `Blob`; on failure re-read the
