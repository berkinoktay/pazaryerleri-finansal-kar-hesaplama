...
/**
 * The Plus tariff-detail Advanced-Filter catalog. Reuses the shared field KEYS +
 * chip adapters (`tariff-filter-fields.ts`) - only the localized labels differ,
 * so category/brand options come from the loaded rows and profit/margin read the
 * PLUS scenario. minMargin is a single lower bound; profit and selection are
 * tri-states whose 'all' case is simply "no chip".
 */
export function usePlusTariffFilterFields(
  categories: readonly string[],
  brands: readonly string[],
): FilterFieldDef[] {
  const t = useTranslations('plusCommissionTariffsPage.filters');
