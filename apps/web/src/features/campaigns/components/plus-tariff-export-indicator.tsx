...
/**
 * The "exported" axis of a Plus tariff's status — separate from the
 * period-validity badge so the two signals stay legible. Exported (saved &
 * downloaded) reads as a quiet success check; not-yet-exported reads as muted
 * "Bekliyor".
 */
export interface PlusTariffExportIndicatorProps {
  exported: boolean;
}

export function PlusTariffExportIndicator({
  exported,
}: PlusTariffExportIndicatorProps): React.ReactElement {
  const t = useTranslations('plusCommissionTariffsPage.status');
