...
/**
 * Profit detail modal for an Advantage scenario (a star tier's target price, the
 * current price, or a custom what-if) — the full income-vs-expense view the seller
 * opens from a profit badge (mirrors the orders page):
 * Satış − Maliyet − Komisyon − Kargo − PSF − Stopaj − Net KDV = Kâr, with the profit +
 * margin tinted by the user's margin scale. Every figure is backend-computed (the
 * estimate engine); this renders, it never calculates — the "−" glyphs are display
 * only. The commission row shows WHERE the rate came from (band vs category).
 */
export function AdvantageTariffBreakdown({
  ...
  // The deduction line-item labels (cost / shipping / platform service / stoppage)
  // are generic profit-formula terms owned by the shared, canonical `profitBreakdown`
  // namespace (the same one ProfitBreakdownCard reads). `productLabelsPage.breakdown`
  // intentionally carries only the Advantage-specific labels, so the per-row labels
  // resolve here to reuse the existing human-authored Turkish rather than duplicate it.
  const tLine = useTranslations('profitBreakdown');
...
/* Mirrors the loaded <dl> anatomy — sale + the 5 deduction rows + net VAT — plus the
   bordered profit/margin footer, so the modal holds its loaded height instead of
   tripling when the estimate lands. Label widths cycle so the rows read as labels of
   differing length, not a barcode wall. */
 ...
 * negative — input VAT > output — which favours the seller), never computed: we strip
 * the served '−' and print our own glyph so Intl does not double-sign.
 */
