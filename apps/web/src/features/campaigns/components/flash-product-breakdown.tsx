...
/**
 * Profit detail modal for a Flash scenario (a flash offer's price, the current price, or a
 * custom what-if) — the full income-vs-expense view the seller opens from a profit badge:
 * Satış − Maliyet − Komisyon − Kargo − PSF − Stopaj − Net KDV = Kâr, with the profit +
 * margin tinted by the user's margin scale. Every figure is backend-computed (the estimate
 * engine); this renders, it never calculates — the "−" glyphs are display only. The
 * commission row shows its resolved rate.
 */
export function FlashProductBreakdown({
  ...
  // The deduction line-item labels (cost / shipping / platform service / stoppage) are
  // generic profit-formula terms owned by the shared, canonical `profitBreakdown`
  // namespace (the same one ProfitBreakdownCard reads). `flashProductsPage.breakdown`
  // intentionally carries only the Flash-specific labels, so the per-row labels resolve
  // here to reuse the existing human-authored Turkish rather than duplicate it.
 ...
 * negative — input VAT > output — which favours the seller), never computed: we strip the
 * served '−' and print our own glyph so Intl does not double-sign.
 */
