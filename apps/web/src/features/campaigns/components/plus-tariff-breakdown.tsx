...
import type { EstimatePlusPriceResult, QuoteBreakdown } from '../api/estimate-plus-item-price.api';
...
export interface PlusTariffBreakdownProps {
  ...
  loading: boolean;
  /** Final profit-row label; defaults to "Kar". Custom-price passes "Tahmini kar". */
  profitLabel?: string;
}

// Deducted GROSS terms, in the authoritative formula order:
// Satis - Maliyet - Komisyon - Kargo - PSF - Stopaj - Net KDV = Kar.
...
/**
 * Profit detail modal for a Plus scenario (the offer's ceiling price, the current
 * price, or a custom what-if) — the full income-vs-expense view the seller opens
 * from a profit badge (mirrors the orders page):
 * Satis - Maliyet - Komisyon - Kargo - PSF - Stopaj - Net KDV = Kar, with the
