...
import { DiscountsListClient } from '@/features/campaigns/components/discount-lists-client';
...
/**
 * Server shell for the İndirimler LIST. Resolves the active org then store (cookie or first,
 * mirroring the dashboard layout) and hands the ids to the client, which lists the saved
 * uploads and owns upload/export/delete.
 */
export default async function DiscountsPage(): Promise<React.ReactElement> {
  ...
  return <DiscountsListClient orgId={activeOrgId ?? null} storeId={activeStoreId ?? null} />;
}
