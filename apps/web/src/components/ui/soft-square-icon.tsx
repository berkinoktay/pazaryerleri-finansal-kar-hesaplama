...
import { type ToneKey, toneSolidClass, toneSurfaceClass } from '@/lib/variants';
...
export const SOFT_SQUARE_ICON_VARIANTS = ['solid', 'soft'] as const;
...
function softSquareIconToneClass(tone: ToneKey, variant: SoftSquareIconVariant): string {
  return variant === 'solid' ? toneSolidClass[tone] : toneSurfaceClass[tone];
}

const softSquareIconVariants = cva(
  'inline-flex shrink-0 items-center justify-center rounded-md [&_svg]:shrink-0',
  {
    variants: {
      size: {
...
export interface SoftSquareIconProps
  ...
  tone?: ToneKey;
  /** `solid` = saturated fill + foreground icon; `soft` = pale surface + tone icon. */
