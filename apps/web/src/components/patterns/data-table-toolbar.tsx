...
import { type FilterFieldDef, type FilterRow } from '@/lib/advanced-filter';
import { cn } from '@/lib/utils';

export interface DataTableToolbarAdvancedFilter {
  /** The table's filterable dimensions (localized labels + facet options). */
  fields: FilterFieldDef[];
  /** Committed filters (usually URL-owned via nuqs). */
  value: FilterRow[];
  /** Commit the full, updated filter set — called on every add / edit / remove / clear. */
...
export interface DataTableToolbarProps<TData> {
  ...
  /**
   * Standard advanced filtering: mounts the `+ Filtre ekle` button in the
   * control row and an auto chip row (FilterChipGroup with click-to-edit +
   * clear-all) beneath it, derived from `value`. Give the toolbar this
...
/**
 * Standard toolbar mounted above a DataTable: search, advanced filtering
 * (the `advancedFilter` config — add button in the control row + an auto
 * chip row beneath), faceted filters, column visibility, and import/export
 * controls. Emits handlers rather than implementing CSV/XLSX serialization
 * here — each feature page decides what shape to serialize (order lines vs
 * flat rows). All copy (search placeholder, clear / import / export labels,
 * column-visibility menu) reads from `t('common.dataTable.toolbar.*')`.
 *
 * @useWhen mounting the standard toolbar above a DataTable for search, advanced/faceted filters, column visibility, and import/export
 */
    ...
    <div className="gap-xs flex flex-col">
      ...
      {advancedFilter !== undefined ? (
        <AdvancedFilterChips
          ...
          onClearAll={() => advancedFilter.onApply([])}
        />
      ) : null}
    </div>
  );
