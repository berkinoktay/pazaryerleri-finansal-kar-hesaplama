...
**103 components synced — the surface is essentially COMPLETE** (pilot 12 + 2a 14 + 3a 8 + 3b 11 + 4 8 + 5 4 + 6 2 + 7 16 + Wave 8 28). The DS is **not a standalone package**: it lives inside the Next.js app (`apps/web/src/components/ui` + `/patterns`). Only a handful of genuinely-unrenderable components remain UNSYNCED (see "Intentionally not synced" below).

- **Wave 8 (28 — everything remaining that renders faithfully):** `ActivityFeed StatGroup IdentityCell UnmatchedVariantBadge ChartFrame RangeInput SoftSquareIcon FilterChipGroup NavGroup BulkActionBar RailWarningCard NotificationBell DateInput ThemeToggleInline ProfitBreakdownCard PromotionIndicator BadgeWithOverflow Combobox Calendar DateRangePicker InfoHint ConfirmDialog ImageModal ImageCell TableEmptyState Sidebar OrgStoreSwitcher NavigationMenu`. Techniques: **domain mocks** — `ProfitBreakdownCard` takes the full `components['schemas']['ProfitBreakdown']` shape (all string decimals: listGross, saleGross, costGross, commissionGross, …, netProfit, saleMarginPct); `PromotionIndicator` takes `{displayName, amountGross}[]` (from `OrderListItem.promotionDisplays`). **react-day-picker** (`Calendar`) bundles + renders Turkish locale; **Sidebar** uses `<Sidebar collapsible="none">` to render inline (avoids the fixed/off-canvas variant); **OrgStoreSwitcher** rendered with `activeStoreId={null}` so it shows the org-initial palette tile instead of `MarketplaceLogo` (which would 404 on `/brands/*.svg`); **next/\* stub** carries `ImageCell`/`ImageModal` (pass a data-URL `src`). All 28 are screenshot-verified (ProfitBreakdownCard, Sidebar, Calendar, OrgStoreSwitcher, NavigationMenu, BadgeWithOverflow, ConfirmDialog, ActivityFeed, ChartFrame all confirmed).

### Intentionally NOT synced (genuinely unrenderable — do not force; they'd ship broken cards)

- `sonner` — the Toaster mount; renders nothing standalone (no trigger).
- `chart` (ChartContainer), `chart-shapes`, `chart-states` — recharts internals / render-helper fns; the concrete charts (BarChart/LineChart/…) already represent them.
- `data-table-toolbar` / `data-table-pagination` / `data-table-row-actions` — need a live `useReactTable` instance; internal to `DataTable` (already synced + visible inside its card).
- `marketplace-logo` / (and `image-cell`'s brand path) — `next/image` pointed at a runtime `/brands/<platform>.svg` public asset that isn't in the DS bundle → 404. (`ImageCell` itself IS synced, via a data-URL src.)
- `sub-nav-list` — labels come from typed i18n `labelKey`s; with arbitrary keys it renders raw key strings.
- `advanced-filter-menu` — `FilterFieldDef`/`FilterRow` builder shapes are non-trivial + nuqs-oriented; low marginal value over `FilterChipGroup` + `FilterTabs`.
- `sync-center` — imports `@/features/sync/*` and needs 3 complex log/trigger/store shapes; the only genuinely feature-coupled one left.
- `context-menu` — Radix ContextMenu has no `open`/`defaultOpen` prop (right-click only) → can't force-render its open state in a static card.
