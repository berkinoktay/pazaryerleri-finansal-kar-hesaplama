...
**59 components synced so far** (pilot 12 + Wave 2a 14 + Wave 3a 8 + Wave 3b 11 + Wave 4 8 charts + Wave 5 4 + Wave 6 2). The DS is **not a standalone package**: it lives inside the Next.js app (`apps/web/src/components/ui` + `/patterns`, ~51 + ~65 files total). The remaining surface is tracked in `docs/plans/2026-06-23-design-sync-expansion-plan.md` (Wave 7 + leftovers).
